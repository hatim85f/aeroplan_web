...
import {
  getMedicalRepProductAssignments,
  createMedicalRepProductAssignment,
  updateMedicalRepProductAssignment,
  closeMedicalRepAssignments,
  todayISO,
} from '../../store/medicalRepProductAssignments/medicalRepProductAssignmentActions';
...
function EditAssignmentModal({ visible, assignment, token, onClose, onSaved }) {
  ...
  const handleSave = async () => {
    ...
    try {
      await updateMedicalRepProductAssignment(
        token,
        assignment._id || assignment.id,
        { startDate, endDate: endDate || undefined, notes: notes || undefined },
      );
      onSaved();
    } catch (e) {
...
/* ── Products Tab ────────────────────────────────────────────────────────── */
function ProductsTab({ token, medicalRepId, isActive, managerRole }) {
  ...
  const fetchAssignments = useCallback(async () => {
    if (!medicalRepId) {
      setListError('No medical rep ID found for this member. Cannot load assignments.');
      setLoading(false);
      return;
    }
  ...
  /* ── load active products when form opens ── */
  useEffect(() => {
  ...
  const handleAssign = async () => {
    ...
    if (!medicalRepId) { setFormError('No medical rep ID — cannot assign.'); return; }

    const productIds = selected.map((p) => p._id || p.id).filter(Boolean);
    if (productIds.length === 0) { setFormError('Could not resolve product IDs.'); return; }

    setAssigning(true); setFormError('');
    try {
      await createMedicalRepProductAssignment(token, {
        medicalRepId,
        productIds,
        startDate,
  ...
  const handleCloseAll = async () => {
    ...
    setClosing(true);
    try {
      await closeMedicalRepAssignments(token, medicalRepId);
      fetchAssignments();
    } catch (e) {
  ...
  return (
    <View style={styles.productsTab}>
      ...
      <EditAssignmentModal
        ...
        onSaved={() => { setEditingAssign(null); fetchAssignments(); }}
      />
      ...
      <View style={styles.card}>
        ...
        {showForm && managerRole && (
          <View style={styles.assignForm}>
            ...
            <Text style={styles.fieldLabel}>
              Products <Text style={{ color: colors.danger }}>*</Text>
              {selected.length > 0 && (
            ...
            <View style={styles.productListBox}>
              {productsLoading ? (
                ...
                <View style={styles.productListLoader}>
                  <Text style={styles.productListLoaderText}>No active products found</Text>
                </View>
              ) : (
          ...
          assignments.map((a) => {
            ...
            const productName =
              typeof a.productId === 'object'
                ...
                : (a.productName || a.product?.productName || a.product?.name || '—');
            return (
...
export default function TeamMemberDetailScreen({
  ...
  const medicalRepId = route?.params?.medicalRepId;
      ...
      {activeTab === 'overview' && (
        <View style={styles.card}>
          ...
          <View style={{ marginTop: 12 }}>
            ...
            <InfoRow label="Rep ID" value={medicalRepId || 'NOT RESOLVED'} mono />
